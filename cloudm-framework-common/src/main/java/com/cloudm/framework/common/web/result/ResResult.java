 ...
 * @version: V1.0
 */
public class ResResult<T> extends BaseResResult<T> {
     ...
     * @return list集合
     */
    public static  <D> ResResult<D> wrapSuccessfulResult(D data, Pageable pageable, int total){
     ...
     * @return list集合
     */
    public static  <D> ResResult<D> wrapSuccessfulResult(D data, Pageable pageable, int total, String message){
        Page springPage = new Page(pageable,total);
        ResResult<D> result = new ResResult<>();
        result.setResult(data);
        result.setSuccess(true);
