 ...
 */
@Slf4j
public class ControllerReqResultInterceptor implements MethodInterceptor {
     ...
     */
    @Override
    public ResResult invoke(final MethodInvocation invocation) throws Throwable {
        ResResult result = null;
        StopWatch watch = new StopWatch();
        watch.start();
        try {
            result = (ResResult) invocation.proceed();
            watch.stop();
    ...
    private ResResult getBaseResult(MethodInvocation methodInvocation) {
        Class<?> returnType = methodInvocation.getMethod().getReturnType();
        ResResult result = (ResResult) generator.newInstance(returnType);
        return result;
    }
    ...
    private ResResult exceptionProcessor(MethodInvocation invocation, Throwable e, Integer errorCode, String message, ServiceError serviceError) {
        ...
        ResResult result = getBaseResult(invocation);
